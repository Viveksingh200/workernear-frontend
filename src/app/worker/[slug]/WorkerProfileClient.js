  ...
  const fetchReviewsList = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setWorker((prev) => ({
          ...prev,
          totalReviews: typeof data.total === "number" ? data.total : data.reviews.length,
          rating: (typeof data.total === "number" && data.total === 0) ? 0 : prev.rating
        }));
      }
    } catch (err) {
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* 1. Profile Card Info (Top left on desktop, top on mobile) */}
        ...
        {/* 2. Leave a Review / Submit a Rating (Right sidebar on desktop, middle above Customer Reviews on mobile) */}
        <div className="lg:col-span-1 order-2">
          {currentUserId !== worker.userId ? (
            <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-sm">
              ...
              {submitError && (
                <p className="text-xs text-red-600 bg-red-50 p-2.5 rounded-lg border border-red-100 font-medium mb-4 animate-fadeIn">
                  {submitError}
                </p>
              )}
              {submitSuccess && (
                <p className="text-xs text-emerald-700 bg-emerald-50 p-2.5 rounded-lg border border-emerald-100 font-medium mb-4 flex items-center gap-1.5 animate-fadeIn">
                    ...
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <div className="flex items-center gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`h-3.5 w-3.5 shrink-0 ${i < myReview.rating ? "fill-amber-400 text-amber-400" : "text-zinc-200"
                              }`}
                          />
                        ))}
                      </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        >
                          <Star
                            className={`h-7 w-7 shrink-0 ${star <= ratingInput ? "fill-amber-400 text-amber-400" : "text-zinc-200"
                              }`}
                          />
                        </button>
                      ))}
        ...
        {/* 3. Customer Reviews List (Bottom left on desktop, bottom on mobile) */}
        <div className="lg:col-span-2 order-3 flex flex-col gap-6">
          <div className="bg-white rounded-2xl border border-gray-150 p-6 sm:p-8 shadow-sm">
                    ...
                    <div className="flex justify-between items-start gap-4 mb-2">
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`h-3.5 w-3.5 shrink-0 ${i < r.rating ? "fill-amber-400 text-amber-400" : "text-zinc-200"
                              }`}
                          />
                        ))}
                      </div>
